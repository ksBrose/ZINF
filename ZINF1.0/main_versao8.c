...
void inicializa_pos_inimigo(Character enemy[], Infos *settings, char mapaJogo[][COLUNAS])
{
    ...
    for (int e = inimigo; e < MAX_INIMIGO; e++)
    {
...
void display_jogo(Character *player, Character enemy[], char mapaJogo[][COLUNAS], Textures *sprites, Infos *settings, Sword *espada)
{
        ...
        case GAMEPLAY:
            {
                ...
                le_nivel(&sprites, &settings, mapaJogo);
                display_jogo(&player, enemy, mapaJogo, &sprites, &settings, &espada);
                move_jogador(&player);
                move_espada(&espada);
                ...
                printf("%f , %f, %d\n",player.rec.x,player.rec.y,player.attacktimer);
